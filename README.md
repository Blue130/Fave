...
All training commands should be launched from the `src/` directory:

```bash
cd src
```

### Stage 1: Pretraining (Manifold Construction)

```bash
python auto_main.py \
  --dataset ml-100k \
  --stage_mode pretrain \
  --ckpt_path "../best for ml-100k/pretrain_best.pt" \
  --epochs 200 \
  --batch_size 512 \
  --hidden_size 128 \
  --lr 0.001 \
...
### Stage 2: Finetuning (Single-Step Consolidation)
...
python auto_main.py \
  ...
  --infer_mask_ratio 0.6 \
  --loss_aux_weight 0.5 \
  --loss_straight_weight 0.1
```

Replace `ml-100k` with `amazon_beauty` or `steam` as needed, and update `--ckpt_path` accordingly.
